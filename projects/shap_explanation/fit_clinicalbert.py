...
class Dataset(torch.utils.data.Dataset):
    ...
    def set_tokenizer(pretrained_model_path):
        ...
        Dataset.tokenizer = AutoTokenizer.from_pretrained(
...
def train(model, train_data, val_data, learning_rate, epochs, max_length,
          batch_size, model_filename):
    ...
    train, val = Dataset(train_data, _max_length=max_length), Dataset(
        val_data, _max_length=max_length)
    ...
    for epoch_num in range(epochs):
        ...
        with torch.no_grad():
            ...
            if best_val_loss is None or (total_loss_val /
                                         len(val_data)) < best_val_loss:
...
def split_train_val_test(df, np_random_seed, split_method):
    ...
    if split_method == 'TAC':
        ...
        drugs_train, drugs_val = np.split(train_drugs,
                                          [int(0.9 * len(train_drugs))])
        ...
        drugs_train, drugs_val, drugs_test = np.split(
            druglist, [int(0.8 * len(druglist)),
                       int(0.9 * len(druglist))])
...
def parse_network_argument(args_network):
    ...
    if args_network.endswith('.pth'):
        ...
        network_code = os.path.split(args_network)[-1].split('_')[0].split(
            '-')[-1]
        if network_code == 'CB':
        ...
        network_code += pretrained_state_ids[os.path.split(pretrained_state)
                                             [-1]]
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--max-length',
        help=
        "maximum number of tokens to use as input for the BERT model, default is smallest power of two that is greater than 2*nwords used in the construction of the reference set",
        type=int,
        default=-1)
    parser.add_argument(
        '--batch-size',
        help=
        "batch size to feed into the model each epoch, will need to balance with max_length to avoid memory errors, default is estimated from a set of runs we've previously run and should work alright",
        type=int,
        default=-1)
    ...
    parser.add_argument(
        '--ifexists',
        help=
        "what to do if model already exists with same parameters, options are 'replicate', 'overwrite', 'quit' - default is 'quit'",
        type=str,
        default='quit')
    parser.add_argument(
        '--network',
        help=
        "path to pretained network, default is 'models/Bio_ClinicalBERT', but you can use other pretrained models or you can use previously saved states.",
        type=str,
    ...
    parser.add_argument(
        '--refsource',
        help=
        "restrict reference data to only one source type, values may be 'all', 'exact', or 'deepcadrme'.",
        type=str,
        default='all')
    parser.add_argument(
        '--split-method',
        help=
        "which method to use to split the data into train/valid/test, default is 90/10/10 and possible values are 24 or TAC",
        type=str,
        default='24')
    ...
    refnwords = int(
        os.path.basename(args.ref).split('nwords')[1].split('_')[0])
    ...
    network_code, network_path, pretrained_state = parse_network_argument(
        args.network)
    ...
    if os.path.exists(final_model_filename):
        print(
            f"Found final model already saved at path: {final_model_filename}")
        if args.ifexists == 'quit':
            print("  Quitting. To run a replicate, use: --ifexists replicate")
            sys.exit(1)
        elif args.ifexists == 'replicate':
            print(
                "  Will run a replicate, checking for any existing replicates..."
            )
            reps = [
                f for f in os.listdir(f'{args.base_dir}/models/')
                if f.find(filename_params) != -1
                and f.lower().find('bestepoch') == -1
            ]
    ...
    training_results = train(model, df_train, df_val, LR, EPOCHS, max_length,
                             batch_size, best_epoch_model_filename)
    ...
    writer.writerow([
        ...
        'valid_accuracy', 'epoch_time', 'epoch_saved'
    ])
