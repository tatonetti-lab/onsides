'''This script trains ChemBERTa (2 different versions) using the SMILES-ADR data from OnSIDES, and then tests on held-out data. It assumes a random train/validation/test split.
For example shell script to run train_chemberta_onsides.py, see s1_randomtvt_10M.sh
Author : JK
'''
...
meta_df.to_csv('onsides_random_split/'+adr_name+'_rs'+str(seed)+'_meta_df.csv',index=False)
