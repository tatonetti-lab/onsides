...
from fit_clinicalbert_TAC import split_train_val
from fit_clinicalbert_TAC import set_up_test

from construct_training_data_TAC import section_names2codes
...
if __name__ == '__main__':
    ...
    parser.add_argument('--examples-train', type=str, required=True, help='Path to the example file that corresponds to the results file (e.g. the reference set file) for the train data.')
    parser.add_argument('--examples-test', type=str, required=True, help='Path to the example file that corresponds to the results file for the TEST data.')
    ...
    train_examplespath = args.examples_train
    dfex_train = pd.read_csv(train_examplespath)

    print(f"Loding reference examples file...", flush=True)
    print(f" ex.shape (before val split): {dfex_train.shape}")

    df_ref = dict()
    df_ref['train'], df_ref['valid'] = split_train_val(dfex_train, np_random_seed)
