...
import fit_clinicalbert_TAC as cb
...
if __name__ == '__main__':
    ...
    print(f"fnnoext is {fnnoext}")
    refset, refsection, refnwords = fnnoext.split('_')[1].split('-')
    print(f"Ref section is {refsection}")
    ...
    train_datapath = f'./data/train_ref{refset}_nwords{refnwords}_clinical_bert_reference_set_{refsection}_TAC.txt'
    if not os.path.exists(train_datapath):
        raise Exception(f"ERROR: No reference train set file found at {train_datapath}")
    test_datapath = f'./data/test_ref{refset}_nwords{refnwords}_clinical_bert_reference_set_{refsection}_TAC.txt'
    if not os.path.exists(test_datapath):
        raise Exception(f"ERROR: No reference test set file found at {test_datapath}")
    ...
    df_test = df_test[df_test['drug'].isin(drugs_test)]
    ...
    if not args.skip_train:
        train_filename = f'{args.base_dir}/results/{prefix}-train_{file_parameters}_TAC.csv'
