...
def generate_subelement(row): # row = row index
	...
	meddra_pt_id = str(row.meddra_id)
	meddra_pt = meddra_dict.get(str(row.meddra_id))
	meddra_pt = meddra_pt.lower()

	# pt or llt?
	meddra_type = dict_meddra_concept.get(row.meddra_id)
	...
	if meddra_type == "LLT":
		pt_match = str(dict_llt_to_pt.get(row.meddra_id)) # this gives the id
...
if __name__ == "__main__":
	...
	parser.add_argument('--threshold', help="threshold for positive result, check Experiment 10 to find the threshold for the validation dataset for the bestepoch model", type=float, required=True)
	...
	# if os.path.exists('./data/xml_output'):
	# 	raise Exception("XML output already exists at ./data/xml_output.")

	if not os.path.exists('./data/xml_output'):
		os.mkdir('./data/xml_output')
