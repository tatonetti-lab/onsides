...
def generate_subelement(row): # row = row index
    id_reaction = 'AR' + str(row[0] + 1) # the id's dont get eval'd so it doesn't matter that much
    id_norm = id_reaction + str(".N") + str(row[0] + 1)
    meddra_pt_id = str(row.pt_meddra_id)
    meddra_pt = meddra_dict.get(str(row.pt_meddra_id))
    meddra_pt = meddra_pt.lower()

    # pt or llt?
    meddra_type = dict_meddra_concept.get(row.pt_meddra_id)
    ...
    if meddra_type == "LLT":
        pt_match = str(dict_llt_to_pt.get(row.pt_meddra_id)) # this gives the id
...
if __name__ == "__main__":
    ...
    parser.add_argument('--threshold', help="threshold for positive result", type=float, required=True)
    ...
    for drug_name in diff_drug_names:
        ...
        dirpath = os.path.join(args.base_dir, 'data', 'xml_output')
        if not os.path.exists(dirpath):
            os.mkdir(dirpath)
        dirpath = os.path.join(dirpath, section)
        if not os.path.exists(dirpath):
            os.mkdir(dirpath)
        resultsname = os.path.split(args.results)[-1].split('.')[0]
        dirpath = os.path.join(dirpath, resultsname)
        if not os.path.exists(dirpath):
            os.mkdir(dirpath)

        filename = os.path.join(dirpath, current_drug + ".xml")
