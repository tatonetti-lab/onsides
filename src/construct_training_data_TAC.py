...
edit 6/28 by cindy: change training to only the 101 training set used for the TAC meeting
"""
...
def main():
    ...
    outfn = f'./data/train_ref{args.method}_nwords{args.nwords}_clinical_bert_reference_set_{args.section}_TAC.txt' # we add a "TAC" at the end to differentiate this from the previous runs
    outfh = open(outfn, 'w')
    ...
    for section in sections:
        ...
        # derive a drug list from the training and testing data provided -- for TAC version, JUST the training data
        ...
        all_drugs = sorted(train_drugs)
        test_trugs = sorted(test_drugs)
        print(f"Found {len(all_drugs)} training drugs with *{suffix} files.")
        print(f"Found {len(test_trugs)} training drugs with *{suffix} files.")
